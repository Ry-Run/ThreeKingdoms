...
func (a *Account) Login(ctx context.Context, req *accountpb.LoginRequest) (*accountpb.LoginReply, error) {
	...
	if err != nil {
		logx.ReportSysErrorWithLoggerContext(ctx, a.log, logx.NewSysLog("account login tech error", err))
		return nil, toRPCError(err)
	}
	if resp != nil && !resp.GetOk() {
		logx.ReportBizWithLoggerContext(ctx, a.log, logx.NewBizLog("account login reject", resp.GetReason(), resp.GetMessage()))
	}
	return resp, nil
}
...
func (a *Account) Register(ctx context.Context, req *accountpb.RegisterRequest) (*accountpb.RegisterReply, error) {
	...
	if err != nil {
		logx.ReportSysErrorWithLoggerContext(ctx, a.log, logx.NewSysLog("account register tech error", err))
		return nil, toRPCError(err)
	}
	if resp != nil && !resp.GetOk() {
		logx.ReportBizWithLoggerContext(ctx, a.log, logx.NewBizLog("account register reject", resp.GetReason(), resp.GetMessage()))
	}
	return resp, nil
}
