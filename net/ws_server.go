...
func (s *WsServer) readMsgLoop() {
	...
	for {
		...
		// 前端发送的是压缩加密过的json
		//1. 解压缩
		secretData, err := util.UnZip(data)
		if err != nil {
		...
		decryptedData, err := util.AesCBCDecrypt(data, []byte(key), []byte(key), openssl.ZEROS_PADDING)
		if err != nil {
...
func (s *WsServer) write(msg *WsMsgResp) {
	...
	// 加密
	key := secretKey.(string)
	encryptedData, err := util.AesCBCEncrypt(marshal, []byte(key), []byte(key), openssl.ZEROS_PADDING)
	if err != nil {
	...
	// 压缩
	zip, err := util.Zip(encryptedData)
	if err != nil {
